...
router.get("/scrape", (req, res) => {
    ...
    request("https://www.bbc.com/news/world", (error, response, body) => {
        if (!error && response.statusCode === 200) {
            // Then, we load that into cheerio and save it to $ for a shorthand selector
            const $ = cheerio.load(body);
            let count = 0;
            // Now, we grab every article:
            $('.macaw .title-link').each(function (i, element) {
                ...
                result.title = $(this)
                .children("h3")
                .children(".title-link__title-text")
                .text();
                result.link = $(this)
                .attr("href");
...
//     db.Article.find({})
//         .then(function(dbArticle) {
//             // If we were able to succesfully find Articles, send them back to the client
//             res.json(dbArticle);
//         })
//         .catch(function(err) {
//             // If an error occurred, send it to the client
